...
export default function JournalView({
	...
	const [journalMode, setJournalMode] = useState<"timeline" | "day">("timeline");
	...
	const combinedItems = useMemo(() => {
		...
		return [...completedTasks, ...entries].sort(
	...
	const filteredItems = useMemo(() => {
		if (journalMode === "day") {
			...
			return combinedItems.filter((item) => getLocalDateStr(item.timestamp) === targetStr);
		}
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const { content: contentAfterTime, timestamp } = parseTimeFromContent(content, baseDate);
		const { content: cleanContent, categoryId: parsedCategoryId } = parseCategoryFromContent(contentAfterTime);
		...
		>
			...
			<div
				className={`px-6 py-3 border-b flex flex-wrap items-center justify-between gap-3 ${
					darkMode ? "border-gray-800/80 bg-gray-950/40" : "border-gray-100 bg-gray-50/50"
				}`}
			>
				{/* Mode Switcher */}
				<div className={`p-0.5 rounded-xl flex gap-1 ${darkMode ? "bg-gray-900" : "bg-gray-100"}`}>
					<button
						type="button"
				...
				<div className="flex items-center gap-2">
					...
					>
						...
						<ChevronDown className={`w-3.5 h-3.5 transition-transform duration-250 ${isCalendarOpen ? "rotate-180" : ""}`} />
					</button>
				</div>
			</div>
				...
				{isCalendarOpen && (
					<motion.div
						...
						className={`border-b overflow-hidden ${
							darkMode ? "bg-gray-900/60 border-gray-800" : "bg-gray-50/70 border-gray-150"
						}`}
					>
						<div className="max-w-2xl mx-auto p-6">
							...
							<div className="grid grid-cols-7 gap-1 text-center mb-2 border-b border-gray-100 dark:border-gray-800/40 pb-1">
								{weekdays.map((day) => (
									<span key={day} className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase py-1">
										{day}
									</span>
								))}
							</div>
			...
			<div className="flex-1 overflow-y-auto px-6 py-8 no-scrollbar scroll-smooth">
				<div className="max-w-2xl mx-auto space-y-10">
					{Object.entries(groupedItems).map(([date, items]) => {
						const dateStr = items.length > 0 ? getLocalDateStr(items[0].timestamp) : "";
						return (
							<div key={date} id={dateStr ? `date-section-${dateStr}` : undefined} className="space-y-4">
								<h3
								...
								<div className="space-y-3 pl-4 border-l-2 border-gray-100 dark:border-gray-800 ml-4">
									{items.map((item) => {
										...
										const dotColor = category ? category.color : (item.type === "Task" ? "#10B981" : "#F97316");
													...
													{editingEntryId === item.id ? (
														...
														<>
															<div className="flex items-center justify-between mb-2">
																<div className="flex items-center gap-2">
																	{category ? (
																		<div className="flex items-center justify-center p-1 rounded-lg" style={{ color: category.color, backgroundColor: `${category.color}15` }}>
					...
					{filteredItems.length === 0 && (
						<div className="text-center py-20 opacity-30">
							...
							<p className="text-sm font-bold uppercase tracking-widest">
								{journalMode === "day" ? "No entries for this day" : "No history yet"}
							</p>
						</div>
					)}
				</div>
